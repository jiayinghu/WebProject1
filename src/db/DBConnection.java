...
public class DBConnection {
	...
	public JSONArray RecommendRestaurants(String userId) {
		try {
			...
			if(count<MIN_RECOMMENDED_RESTAURANTS){
				Set<String> moreCategory=getMoreCategory(allCategories);
				Set<String> moreRestaurants=new HashSet<>();
				for(String category:moreCategory){
					Set<String> set=getBusinessId(category);
					moreRestaurants.addAll(set);
				}
				for(String restaurant:moreRestaurants){
					if(!visitedRestaurants.contains(restaurant)){
						diff.add(getRestaurantsById(restaurant));
						count++;
						if(count>=MAX_RECOMMENDED_RESTAURANTS){
							break;
						}
	...
	private Set<String> getMoreCategory(Set<String> categories){
			Set<String> moreCategory=new HashSet<>();
			for(String category:categories){
			   moreCategory.addAll(getMoreCategory(category));
			}
			return moreCategory;
		
	}
	private Set<String> getMoreCategory(String category){
	        Set<String> moreCategory=new HashSet<>();
	        try{
	           Statement stmt=conn.createStatement();
	           ...
	           while(result.next()){
	        	   ...
	        	   for(String c:categories){
	        		   ...
	        		   if(!c_trim.equals(category)&&!c_trim.equals("Restaurants")){
	        			   moreCategory.add(c_trim);
	        		   }
	        	   }
	           }
	        }catch (Exception e){
